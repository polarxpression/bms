...
class AppState extends ChangeNotifier {
  ...
  final CollectionReference _collection = FirebaseFirestore.instance.collection('batteries');
  final CollectionReference _settingsCollection = FirebaseFirestore.instance.collection('settings');
  final CollectionReference _mapsCollection = FirebaseFirestore.instance.collection('maps');
  ...
  BatteryMap? get currentMap => _maps.isEmpty || _currentMapId == null 
      ? null 
      : _maps.firstWhere((m) => m.id == _currentMapId, orElse: () => _maps.first);
  ...
  void _initRealtimeUpdates() {
    ...
    // Settings
    _settingsSubscription = _settingsCollection.doc('config').snapshots().listen((snapshot) {
      if (snapshot.exists) {
    ...
    _mapsCollection.snapshots().listen((snapshot) {
      ...
      } else if (_currentMapId == null || !_maps.any((m) => m.id == _currentMapId)) {
  ...
  Future<void> _migrateLegacyMapData() async {
    try {
      ...
      if (migrationDoc.exists && (migrationDoc.data() as Map)['legacyMapMigrated'] == true) return;

      final legacyCollection = FirebaseFirestore.instance.collection('battery_map');
      final legacyDocs = await legacyCollection.get();
      
      if (legacyDocs.docs.isEmpty) {
        // No data to migrate, just mark as done
        await _settingsCollection.doc('migration').set({'legacyMapMigrated': true}, SetOptions(merge: true));
        return;
      }

      // We need a target map. Use the first one (likely 'Mapa Principal')
      final targetMapId = _maps.first.id;
      final targetCellsRef = _mapsCollection.doc(targetMapId).collection('cells');
  ...
  void selectMap(String mapId) {
    ...
    _cellsSubscription?.cancel();
    _cellsSubscription = _mapsCollection.doc(mapId).collection('cells').snapshots().listen((snapshot) {
      final Map<String, String> newMap = {};
      for (var doc in snapshot.docs) {
        final data = doc.data();
        if (data.containsKey('x') && data.containsKey('y') && data.containsKey('batteryId')) {
  ...
  Future<void> updateMap(String id, String name, String purpose) async {
    await _mapsCollection.doc(id).update({
      'name': name,
      'purpose': purpose,
    });
  }
  ...
  Future<void> refreshData() async {
    ...
    if (_currentMapId != null) {
        final cellsSnapshot = await _mapsCollection.doc(_currentMapId).collection('cells').get();
        final Map<String, String> newMap = {};
  ...
  Future<void> removeBatteryFromMap(int x, int y) async {
    if (_currentMapId == null) return;
    final docId = 'cell_${x}_$y';
    await _mapsCollection.doc(_currentMapId).collection('cells').doc(docId).delete();
  }
  ...
  int getStockForBattery(Battery battery) {
    ...
    final nameKey = '${removeDiacritics(battery.brand.trim().toLowerCase())}|${removeDiacritics(battery.model.trim().toLowerCase())}';
    
    int total = 0;
    for (var other in _batteries) {
      // Only count stock from items that are NOT in Gondola
      final isGondola = other.location.toLowerCase().contains('gondola') || other.location.toLowerCase().contains('gôndola');
      ...
      if (bc.isNotEmpty && other.barcode.trim() == bc) {
        ...
        // Fallback name match
        final k = '${removeDiacritics(other.brand.trim().toLowerCase())}|${removeDiacritics(other.model.trim().toLowerCase())}';
      ...
      for (var doc in query.docs) {
        ...
        if (mapRef != null) {
          ...
          final map = _maps.firstWhere((m) => m.id == mapId, 
              orElse: () => BatteryMap(id: 'unknown', name: 'Mapa Desconhecido', purpose: ''));
  ...
  int get totalBatteries => _batteries.fold(0, (sum, item) => sum + item.quantity);
  ...
  List<Battery> get lowStockBatteries {
    ...
    for (var b in gondolaItems) {
      final limit = b.gondolaLimit > 0 ? b.gondolaLimit : _defaultGondolaCapacity;
      if (limit <= 0) continue;
      ...
      if (b.gondolaQuantity <= (limit / 2.0)) {
         ...
         if (availableStock > 0) {
           ...
           suggestions.add(Battery(
             ...
             barcode: b.barcode,
             imageUrl: b.imageUrl,
             quantity: availableStock, // VISUAL: Total available stock found by barcode
  ...
  List<Battery> get externalBuyBatteries {
    ...
    for (var b in _batteries) {
      ...
      if (bc.isNotEmpty) {
        ...
        key = 'name:${removeDiacritics(b.brand.trim().toLowerCase())}|${removeDiacritics(b.model.trim().toLowerCase())}';
      }
    ...
    groups.forEach((key, items) {
      ...
      for (var b in items) {
        ...
        if (b.minStockThreshold > maxMinStock) maxMinStock = b.minStockThreshold;
        if (b.gondolaLimit > maxGondolaLimit) maxGondolaLimit = b.gondolaLimit;
      ...
      final effectiveThreshold = maxMinStock > 0 ? maxMinStock : _defaultMinStockThreshold;
      ...
      if (shouldAdd) {
        buyList.add(Battery(
          ...
          quantity: stockToCheck, // VISUAL: Effective Stock used for decision (Stock or Gondola fallback)
          gondolaQuantity: totalGondola, 
          location: primary.location,
          minStockThreshold: effectiveThreshold, // VISUAL: Target Threshold
          gondolaLimit: maxGondolaLimit > 0 ? maxGondolaLimit : _defaultGondolaCapacity,
  ...
  Future<void> addBattery(Battery battery) async => await _collection.add(battery.toMap());
  Future<void> updateBattery(Battery updated) async => await _collection.doc(updated.id).update(updated.toMap());
  Future<void> deleteBattery(String id) async => await _collection.doc(id).delete();

  Future<void> adjustQuantity(Battery battery, int delta) async {
    final newQty = (battery.quantity + delta).clamp(0, 9999);
    await _collection.doc(battery.id).update({'quantity': newQty, 'lastChanged': Timestamp.now()});
  }
  
  // NEW: Adjust gondola quantity
  Future<void> adjustGondolaQuantity(Battery battery, int delta) async {
    final limit = battery.gondolaLimit > 0 ? battery.gondolaLimit : _defaultGondolaCapacity;
    final newQty = (battery.gondolaQuantity + delta).clamp(0, limit);
    await _collection.doc(battery.id).update({'gondolaQuantity': newQty, 'lastChanged': Timestamp.now()});
  }
  ...
  Future<void> moveToGondola(Battery battery, int amount) async {
    ...
    final realBattery = _batteries.firstWhere((b) => b.id == battery.id, orElse: () => battery);
    
    final limit = realBattery.gondolaLimit > 0 ? realBattery.gondolaLimit : _defaultGondolaCapacity;
    
    // 1. Identify Siblings (Same Product)
    // Priority: Barcode match. Fallback: Name match.
    final bc = realBattery.barcode.trim();
    final nameKey = '${removeDiacritics(realBattery.brand.trim().toLowerCase())}|${removeDiacritics(realBattery.model.trim().toLowerCase())}';
    ...
    final siblings = _batteries.where((b) {
      ...
      if (bc.isEmpty || b.barcode.trim().isEmpty) {
         final k = '${removeDiacritics(b.brand.trim().toLowerCase())}|${removeDiacritics(b.model.trim().toLowerCase())}';
         return k == nameKey;
      }
    ...
    for (var s in siblings) totalStock += s.quantity;
    ...
    final newGondolaQty = (realBattery.gondolaQuantity + safeAmount).clamp(0, limit);
    ...
    if (realBattery.quantity > 0) {
      final deduct = (realBattery.quantity >= remainingToDeduct) ? remainingToDeduct : realBattery.quantity;
      await _collection.doc(realBattery.id).update({
    ...
    if (remainingToDeduct > 0) {
      ...
      for (var s in siblings) {
        ...
        if (s.quantity > 0) {
          final deduct = (s.quantity >= remainingToDeduct) ? remainingToDeduct : s.quantity;
          await _collection.doc(s.id).update({
            'quantity': s.quantity - deduct
          });
          remainingToDeduct -= deduct;
        }
      }
